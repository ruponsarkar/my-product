...
const generateAccessToken = (userId: string, tenantId: string) => {
  return jwt.sign({ id: userId, tenantId }, JWT_SECRET, {
    expiresIn: "1d",
  });
};

const generateRefreshToken = (userId: string, tenantId: string) => {
  return jwt.sign({ id: userId, tenantId }, JWT_REFRESH_SECRET, { expiresIn: "7d" });
};
...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    const accessToken = generateAccessToken(user._id.toString(), tenant.tenantId);
    const refreshToken = generateRefreshToken(user._id.toString(), tenant.tenantId);
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    const tenant = await findTenant({ email, tenantId, tenantSlug });
    if (!tenant) {
...
export const refreshToken = async (req: Request, res: Response) => {
  try {
    ...
    const newAccessToken = generateAccessToken(decoded.id, decoded.tenantId);
