...
`;
const modelPath = path_1.default.join(process.cwd(), "src", "models", `${lowerName}.model.ts`);
fs_1.default.mkdirSync(path_1.default.dirname(modelPath), { recursive: true });
...
// ================== Controller ==================
if (createController) {
    const controllerPath = path_1.default.join(process.cwd(), "src", "controllers", `${lowerName}.controller.ts`);
...
`;
    ...
    if (fs_1.default.existsSync(controllerPath)) {
        ...
        fs_1.default.writeFileSync(controllerPath, controllerTemplate);
        console.log(`✅ Controller created: src/controllers/${lowerName}.controller.ts`);
    }
}
// ================== Route ==================
if (createRoute) {
    const routePath = path_1.default.join(process.cwd(), "src", "routes", `${lowerName}.routes.ts`);
...
`;
    fs_1.default.mkdirSync(path_1.default.dirname(routePath), { recursive: true });
    if (fs_1.default.existsSync(routePath)) {
        console.error(`⚠️ Route already exists: src/routes/${lowerName}.routes.ts`);
    }
    else {
        fs_1.default.writeFileSync(routePath, routeTemplate);
    ...
    if (fs_1.default.existsSync(routesIndexPath)) {
        let content = fs_1.default.readFileSync(routesIndexPath, "utf-8");
        ...
        fs_1.default.writeFileSync(routesIndexPath, content);
